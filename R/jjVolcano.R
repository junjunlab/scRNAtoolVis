...
globalVariables(c('p_val', 'p_val_adj', 'type', 'type2'))
   jjVolcano <- function(diffData = NULL,
      ...
      diff.marker <- diff.marker %>%
        dplyr::mutate(type = ifelse(avg_log2FC >= difflog2FC.point, "sigUp",
                                    ifelse(avg_log2FC <= -difflog2FC.point, "sigDown", "sigStable"))) %>%
      ...
      top.marker.tmp <- diff.marker %>%
        dplyr::filter(type %in% c("sigUp", "sigDown")) %>%
      ...
      if(col.type == "updown"){
        p2 <- p1 +
          ...
          ggplot2::scale_color_manual(values = c(sigDown = aesCol[1], sigUp = aesCol[2], sigStable = aesCol[3]))
      } else if(col.type == "adjustP"){
        p2 <- p1 +
          # add point
          ggplot2::geom_jitter(ggplot2::aes(color = type2), size = pSize) +
          ggplot2::scale_color_manual(values = c(`adjust Pvalue >= 0.05` = aesCol[2], `adjust Pvalue < 0.05` = aesCol[1]))
      }
      # theme details
      p3 <- p2 +
        ...
        ggplot2::theme(legend.text = ggplot2::element_text(hjust = 0, lineheight = 1))
      ...
      p4 <- p3 +
        ...
        ggrepel::geom_text_repel(data = top.marker %>%
                                 ...
                                 max.overlaps = 60,segment.size = 0.3, box.padding = 0.1,
                                 ...)
      ...
      if(polar == TRUE){
        ...
        if(flip == TRUE){
          ...
          p5 <- p4 +
            ggplot2::scale_y_continuous(n.breaks = 6) +
            ggplot2::geom_text(ggplot2::aes(x = cluster,y = 0,label = cluster), size = cluster_size) +
